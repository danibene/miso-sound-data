...
def is_url(x):
 ...
 def download_from_url(in_path, out_dir_path):
  ...
  with urlopen(in_path) as r:
      ...
      pydub.AudioSegment.from_file(r).export(aud, pathlib.Path(in_path).suffix.split(".")[1])
  aud.seek(0)
...
def normalize(y, level=0.0):
  ...
  if level is not None:
    return y * np.sqrt( (len(y) * np.square(10**(level / 10))) / np.sum(np.square(y)) )
  else:
    return y
...
class MisoSoundLoader:
    ...
    def get_paths(self, 
      ...
      original_audio_out_dir_path = str(pathlib.Path(root_out_path, original_audio_out_dir_name))
      processed_audio_out_dir_path = str(pathlib.Path(root_out_path, processed_audio_out_dir_name))
      all_paths = {}
      audio_paths = {}
      label_paths = {}
      ...
      for f in audio_f_list:
        id = f["id"]
        audio_paths[id] = {"original_audio_in_file_path": f["links"]["self"],
                        "original_audio_out_dir_path": original_audio_out_dir_path,
                        "segment_in_file_path": segment_in_dir_path + r"/" + str(id) + "_segment.txt",
                        "processed_audio_out_file_path": str(pathlib.Path(processed_audio_out_dir_path, str(id) + processed_audio_name_end + ".wav"))}
        label_paths[id] = {"label_out_dir_path": label_out_dir_path,
                           "label_in_file_path": label_in_dir_path + r"/" + str(id) + "_labels.txt"}
      info_paths = {"original_metadata_in_file_path": [f for f in original_audio_d["files"] if ".csv" in f["key"]][0]["links"]["self"],
    ...
    def get_audio(self, save_original=True, save_processed=True, return_audio=True, segment_processed=True, process_func=process_audio):
      """Load, segment and process audio"""
      audio_paths = self.get_paths()["audio"]
      audio = []
      for id, paths in audio_paths.items():
        if save_original:
         y = download_from_url(in_path=paths["original_audio_in_file_path"], out_dir_path=paths["original_audio_out_dir_path"])
         sr = None
        else:
          y, sr = load_audio_from_url(in_path=paths["original_audio_in_file_path"])
    ...
    def get_info(self, save=True):
      ...
      if save:
        download_from_url(in_path=info_paths["original_metadata_in_file_path"], out_dir_path=info_paths["original_metadata_out_dir_path"])
      return info_df
    ...
    def get_labels(self, save=True):
      ...
      for id, paths in label_paths.items():
        ...
        labels.append(label)
        if save:
          download_from_url(in_path=paths["label_in_file_path"], out_dir_path=paths["label_out_dir_path"])
      label_df = pd.concat(labels)
      label_df.columns = ["start", "stop", "full_label", "id"]
      salience_d = {"C1": "foreground", "C2": "background"}
      label_df["salience"] = [salience_d[v.split("-")[0]] for v in label_df["full_label"].values]
